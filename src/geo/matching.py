...
class GeoMatcher:
    ...
    def load_catalog(self):
        if not os.path.exists(self.catalog_path):
            logger.warning(
                f"Territory catalog not found at {self.catalog_path}. Matching will fail."
            )
            return
    ...
    def match_territory(
        self, provincia_input: str, canton_input: str, threshold: int = 85
    ):
        """
        ...
        if prov_norm_input in self.provinces:
            ...
            best_prov = process.extractOne(
                prov_norm_input, self.provinces, scorer=fuzz.ratio
            )
        ...
        if canton_norm_input in valid_cantons:
            ...
            # Fuzzy match canton
            best_canton = process.extractOne(
                canton_norm_input, valid_cantons, scorer=fuzz.ratio
            )
